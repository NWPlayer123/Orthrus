//! This file is for shared functions across multiple modules in this crate. The filename may change, and it's
...
// Finds the longest match in a 0x1000-byte sliding window, searching front-to-back with a minimum match size
// of 3 bytes. The algorithm is similar to the one described in section 4 of RFC 1951
// (https://www.rfc-editor.org/rfc/rfc1951.html#section-4), using a chained hash table of 3-byte sequences to
// find matches. Each character in the window is identified by its position & 0xFFF (like in a circular
// buffer).
...
impl Window<'_> {
    ...
    pub(crate) fn advance(&mut self) {
        ...
        if self.input_pos >= WINDOW_SIZE {
            self.hash_start =
                update_hash(self.hash_start, self.input[self.input_pos - WINDOW_SIZE + MIN_MATCH - 1]);
    ...
    // Move the window forward the input position, and seach the window back-to-front for a match at most
    // `max_match_length` bytes long, returning the offset and length of the longest match found. Successive
    // searches can only be performed at increasing input positions.
